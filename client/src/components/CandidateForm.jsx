...
const CandidateForm = () => {
  ...
  return (
    /* MOBILE FIX: Changed h-screen to min-h-screen and removed overflow-hidden. Added py-20 for mobile spacing */
    <section className="min-h-screen flex flex-col justify-center bg-[#F8FAFC] py-20 lg:py-0">
      <div className="container mx-auto px-6 lg:px-12">
        {/* MOBILE FIX: gap-12 ensures spacing between stacked elements */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
          ...
          >
            ...
            {/* MOBILE FIX: Smaller text size for mobile heading */}
            <h1 className="text-3xl lg:text-[52px] font-black text-[#071952] leading-[1.05] mb-5 tracking-tighter">Connect with Companies<br /><span className="text-[#0B57D0]"> That Value Your Skills.</span></h1>
            <p className="text-[14px] text-slate-600 mb-2 leading-relaxed font-medium max-w-md">Skip the application queues. One profile gets you noticed by 200+ high-growth companies.</p>
          </motion.div>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="bg-white p-6 lg:p-8 rounded-[32px] shadow-[0_20px_60px_rgba(7,25,82,0.06)] border border-slate-100"
          >
            <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      {isOpen && (
                        <>
                          ...
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: 5 }}
                            className="absolute z-20 w-full bg-white border border-slate-100 rounded-2xl shadow-xl overflow-hidden p-1.5"
                          >
                            {['Frontend Developer', 'Backend Developer', 'Full Stack Developer'].map((role) => (
                              <button
                                key={role}
                                type="button"
                                onClick={() => { handleInputChange('role', role); setIsOpen(false); }}
                                className={`w-full text-left px-4 py-2 rounded-xl text-[12px] font-bold transition-all ${formData.role === role ? 'bg-[#EEF2FF] text-[#0B57D0]' : 'text-slate-600 hover:bg-slate-50'}`}
                              >
                                {role}
                              </button>
                            ))}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={skillInput} onChange={e => setSkillInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSkill()}
                    placeholder="Add skill..." className={`flex-1 px-4 py-1.5 bg-slate-50 border rounded-xl focus:outline-none focus:border-[#0B57D0] text-[12px] ${errors.skills ? 'border-red-400 bg-red-50' : 'border-slate-100'}`} />
              ...
              <div>
                <div className="flex justify-between items-center mb-1 ml-1">
                    ...
                    <span className="text-[8px] font-bold text-slate-400 uppercase">Max 2MB</span>
                </div>
                <label className={`group relative flex items-center justify-center w-full h-14 border-2 border-dashed rounded-xl cursor-pointer transition-all ${errors.file ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-slate-50'}`}>
                  <div className="flex items-center gap-2">
                    {file ? (
                      ...
                      <><Upload size={14} className="text-slate-400 group-hover:text-[#0B57D0]" /><span className="text-[10px] text-slate-500 font-bold">Upload Resume</span></>
                    )}
                  </div>
              ...
              <div>
                ...
                <textarea rows="2" value={formData.message} onChange={e => handleInputChange('message', e.target.value)}
                  placeholder="Expertise summary..." className={`w-full px-4 py-2 bg-slate-50 border rounded-xl focus:outline-none focus:border-[#0B57D0] transition-all resize-none text-[13px] font-medium ${errors.message ? 'border-red-400 bg-red-50' : 'border-slate-100'}`}
                ></textarea>
              ...
              <button
                ...
                className={`w-full py-3.5 ${isSubmitted ? "bg-green-500" : "bg-[#0B57D0]"} text-white font-black rounded-xl shadow-lg transition-all active:scale-95 text-[13px] flex items-center justify-center gap-2 uppercase tracking-widest`}
              >

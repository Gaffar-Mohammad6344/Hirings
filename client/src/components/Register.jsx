...
//   return (
//     <div className="reg-container mt-7">
//       <div className="reg-card">
//         <div className="reg-left">
//           <div className="reg-logo"><Sparkles size={20} /> <span>Hirings</span></div>
//           <div className="reg-hero">
//             <span className="badge">SECURITY FIRST</span>
//             <h1>Secure Your<br /><span>Admin Portal.</span></h1>
//             <p>Verification of both personal and corporate identity is mandatory.</p>
//           </div>
//         </div>

//         <div className="reg-right">
//           <div className="form-container">
//             <h2 className="title">Join Hirings</h2>
//             <p className="subtitle">Already have an account? <Link to="/login" className="text-blue-600 font-bold">Sign In</Link></p>

//             <form onSubmit={handleSubmit}>
//               <div className="grid grid-cols-2 gap-4 mb-3">
//                 <InputField label="Full Name" icon={<User size={14}/>} name="fullName" placeholder="Jane Doe" onChange={handleChange} />
//                 <InputField label="Phone" icon={<Phone size={14}/>} name="phone" placeholder="9876543210" onChange={handleChange} />
//               </div>

//               <InputField 
//                 label="Personal Email" icon={<Mail size={14}/>} name="personalEmail" placeholder="John@gmail.com" onChange={handleChange} disabled={pVerified}
//                 rightIcon={pVerified ? <CheckCircle size={16} className="text-green-500"/> : <button type="button" onClick={() => triggerOtp('p')} className="v-btn">{loading.p ? "..." : "Verify"}</button>}
//               />
//               {pSent && !pVerified && (
...
//               <InputField 
//                 label="Company Email" icon={<Building2 size={14}/>} name="companyEmail" placeholder="demo@hire.in" onChange={handleChange} disabled={cVerified}
//                 rightIcon={cVerified ? <CheckCircle size={16} className="text-green-500"/> : <button type="button" onClick={() => triggerOtp('c')} disabled={!pVerified} className="v-btn">{loading.c ? "..." : "Verify"}</button>}
//               />
//               {cSent && !cVerified && (
//                 <div className="otp-row">
...
//               <InputField 
//                 label="Password" icon={<Lock size={14}/>} name="password" placeholder="••••••••" type={showPassword ? "text" : "password"} onChange={handleChange}
//                 rightIcon={showPassword ? <EyeOff size={14} className="cursor-pointer" onClick={()=>setShowPassword(false)}/> : <Eye size={14} className="cursor-pointer" onClick={()=>setShowPassword(true)}/>}
//               />

//               <button type="submit" disabled={!pVerified || !cVerified || loading.submit} className="submit-btn">
//                 {loading.submit ? <Loader2 className="animate-spin mx-auto" size={18} /> : "Create Free Account →"}
//               </button>
...
//       <style>{`
//         .reg-container { height: 100vh; display: flex; align-items: center; justify-content: center; background: #f8fafc; font-family: 'Inter', sans-serif; overflow: hidden; }
//         .reg-card { display: flex; width: 1000px; height: 600px; background: white; border-radius: 30px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.1); border: 1px solid #f1f5f9; }
//         .reg-left { flex: 0.8; background: #071952; padding: 40px; color: white; display: flex; flex-direction: column; justify-content: space-between; }
//         .reg-logo { display: flex; align-items: center; gap: 10px; font-weight: 900; font-size: 20px; }
//         .reg-hero h1 { font-size: 38px; font-weight: 900; line-height: 1.1; margin-top: 20px; }
//         .reg-hero span { color: #60a5fa; }
//         .badge { color: #60a5fa; font-size: 10px; font-weight: 800; letter-spacing: 2px; }
//         .reg-right { flex: 1.2; padding: 30px 50px; display: flex; align-items: center; }
//         .form-container { width: 100%; }
//         .title { font-size: 24px; font-weight: 900; color: #071952; margin-bottom: 2px; }
//         .subtitle { font-size: 12px; color: #64748b; margin-bottom: 20px; }
//         .v-btn { background: #071952; color: white; border: none; padding: 4px 12px; border-radius: 6px; font-size: 10px; font-weight: 800; cursor: pointer; }
//         .v-btn:disabled { background: #cbd5e1; cursor: not-allowed; }
//         .otp-row { display: flex; gap: 10px; background: #f0f7ff; padding: 8px; border-radius: 12px; margin: 4px 0 10px 0; border: 1px dashed #0B57D0; }
//         .otp-input { flex: 1; background: transparent; border: none; outline: none; font-size: 11px; font-weight: 600; color: #071952; }
//         .confirm-btn { background: #10b981; color: white; border: none; font-size: 9px; font-weight: 800; padding: 4px 12px; border-radius: 6px; cursor: pointer; }
//         .submit-btn { width: 100%; height: 48px; background: #071952; color: white; border: none; border-radius: 12px; font-weight: 800; cursor: pointer; margin-top: 15px; transition: 0.3s; }
//         .submit-btn:hover { background: #0b57d0; }
//         .submit-btn:disabled { background: #cbd5e1; cursor: not-allowed; }
//         .instruction-box { margin-top: 15px; display: flex; align-items: center; justify-content: center; gap: 8px; font-size: 11px; color: #475569; font-weight: 600; }
...
// const InputField = ({ label, icon, rightIcon, ...props }) => (
//   <div style={{ marginBottom: "12px" }}>
//     <label style={{ fontSize: "9px", fontWeight: "900", color: "#64748b", textTransform: "uppercase", display: 'block', marginBottom: '4px' }}>{label}</label>
//     <div style={{ display: "flex", alignItems: "center", border: "1.5px solid #f1f5f9", borderRadius: "10px", padding: "0 12px", height: "42px", background: "#fff" }}>
//       <span style={{ color: "#94a3b8", marginRight: "10px" }}>{icon}</span>
//       <input {...props} style={{ flex: 1, border: "none", outline: "none", fontSize: "12px", fontWeight: "600", color: "#071952", background: 'transparent' }} />
//       {rightIcon}
//     </div>
//   </div>
// );
  ...
  return (
    <div className="register-page-wrapper" >
      <div className="main-content shadow-2xl">
        {/* LEFT PANEL - MATCHED TO LOGIN PAGE */}
      ...
      <style>{`
        .register-page-wrapper { 
          ...
          padding-top: 120px; /* SPACE AFTER NAVBAR */
        }
        ...
        @media(max-width: 900px) {
          ...
          .main-content { max-width: 420px; height: auto; padding: 25px; }
          .register-page-wrapper { padding-top: 40px; }
        }
      `}</style>
    </div>
  );
